...
(module regexp racket/base
  ...
  (define-RE OWS (* (chars #\space #\tab)))
  (define-RE TCHAR (chars alpha digit "!#$&'*+-.^_`|~"))
  (define-RE TOKEN (+ TCHAR))
  (define-RE lower-TCHAR (chars (intersect TCHAR (complement upper)))) ;; (chars lower digit "!#$&'*+-.^_`|~"))
  (define-RE lower-TOKEN (+ lower-TCHAR)))
...
(module url racket/base
  ...
  ;; Approximations
  (define-RE ip4-rx (cat (repeat (chars digit) 1 3) "." (repeat (chars digit) 3)))

  (define-RE h16 (repeat (chars xdigit) 1 4))
  (define-RE h16: (cat h16 ":"))
  (define-RE ls32 (or (cat h16 ":" h16) ip4-rx))
  ...
  (define-RE ip6-exact-rx
    (or (cat                       (repeat h16: 6) ls32)
        ...
        (cat (repeat h16: 1 7) ":")))
  (define-RE ip6-approx-rx (cat (? ":") (repeat h16: 0 7) (or ls32 h16 ":")))
  (define-RE ip6-rx (cat "[" ip6-exact-rx "]"))

  (define-RE unreserved-rx (chars alpha digit "-._~"))
  (define-RE pct-encoded-rx (cat "%" (repeat (chars xdigit) 2)))
  (define-RE sub-delims-rx (chars "!$&'()*+,;="))
  (define-RE reg-name-rx (* (or unreserved-rx pct-encoded-rx sub-delims-rx)))

  (define-RE host-rx (or ip4-rx ip6-rx reg-name-rx))
  (define-RE host+port-rx (cat (report host-rx) ":" (report (+ (chars digit)))))
  ...
  (define (parse-connect-target loc)
    (match (regexp-match (px^$ host+port-rx) loc)
      [(list _ host port-bs)
       (list host (string->number (bytes->string/latin-1 port-bs)))]
      [else #f]))

  (define (check-http-url who loc [orig loc] #:what [what "URL"])
    (match loc
      [(url scheme user host port path-abs? path query fragment)
       (define (bad msg) (error who "bad ~a, ~a\n  URL: ~e" what msg orig))
       (unless scheme (bad "missing scheme"))
       (define scheme* (normalize-http-scheme scheme))
       (unless scheme* (bad "expected \"http\" or \"https\" for scheme"))
       (when user (bad "contains userinfo"))
       (unless host (bad "missing host"))
       (define host* (string->immutable-string (string-downcase host)))
       (unless path-abs? (bad "path is not absolute"))
