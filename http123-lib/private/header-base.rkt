...
(define-RE FIELD-VCHAR (chars [#x21 #x7E]))
;;(define-RE FIELD-CONTENT (cat FIELD-VCHAR (? (cat (+ (chars " \t")) FIELD-VCHAR))))
;;(define-RE FIELD-VALUE (* FIELD-CONTENT))
(define-RE FIELD-VALUE (or "" (cat FIELD-VCHAR (* (cat (* (chars " \t")) FIELD-VCHAR)))))

(define-RE HEADER-START (cat (report TOKEN) ":"))
(define-RE HEADER-FIELD (cat HEADER-START OWS (report FIELD-VALUE) OWS))

(define-RE TOKEN+ (cat TOKEN (* (cat OWS "," OWS TOKEN))))

(define-RE qdtext (chars #\space #\tab #x21 [#x23 #x5B] [#x5D #x7E]))
(define-RE quoted-pair (cat "\\" (chars #\space #\tab [#x21 #x7E])))
(define-RE quoted-string (cat "\"" (* (or qdtext quoted-pair)) "\""))
...
(define (check-header-field hf)
  ...
  (match hf
    ...
    [(? bytes?)
     (match (regexp-match (px^$ HEADER-FIELD) hf)
       [(list _ key val)
        (list (check-header-field-key key) (bytes->immutable-bytes val))]
       [_ (bad)])]
    [(? string?)
     (match (regexp-match (px^$ HEADER-FIELD) (string->bytes/utf-8 hf))
       [(list _ key val)
        (list (check-header-field-key key) (bytes->immutable-bytes val))]
       [_ (bad)])]
    [_ (bad)]))
...
(define (check-header-field-key key0)
  ...
  (let loop ([key key0])
    (match key
      ...
      [(? bytes? (regexp (px^$ lower-TOKEN))) (imm key)]
      [(? string? (regexp (px^$ lower-TOKEN))) (imm (string->bytes/latin-1 key))]
      [(? bytes? (regexp (px^$ TOKEN))) (loop (bytes->string/latin-1 key))]
      [(? string? (regexp (px^$ TOKEN))) (loop (string-downcase key))]
...
(define (check-header-field-value value)
  (match value
    [(? bytes? (regexp (px^$ OWS (report FIELD-VALUE) OWS) (list _ field-value)))
     (bytes->immutable-bytes field-value)]
    [(? string? (regexp (px^$ OWS (report FIELD-VALUE) OWS) (list _ field-value)))
     (bytes->immutable-bytes field-value)]
...
(define (header-key-name? s)
  (regexp-match? (px^$ lower-TOKEN) s))

(define (header-key-name-ci? s)
  (regexp-match (px^$ TOKEN) s))
